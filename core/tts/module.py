...
maintainability. Contains the TTSModule class, which orchestrates the
available TTS backends (edge-tts, Piper, espeak-ng) to produce British
butler voice output.

Configuration constants live in core/tts/config.py and the backend
protocol/registry in core/tts/protocol.py; both are re-exported from
